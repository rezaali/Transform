...
{
	createDirectories( getAppSupportPath() );

	/*
	CI_LOG_D( getResourcesPath( "" ) );
	auto colorsPath = getAssetPath( "colors.png" );
	fs::copy( colorsPath, getAppSupportPath( "colors.png" ) );
	CI_LOG_D( getAssetPath( "colors.png" ) );
	auto assetDirectories = getAssetDirectories();
	for( auto dir : assetDirectories ) {
		CI_LOG_D( "asset directory: " );
		CI_LOG_D( dir );
	}
	*/

	CI_LOG_D( getAppSupportPath() );
	CI_LOG_D( "SETUP ASSETS DIRECTORIES" );
	createAssetDirectories();
	CI_LOG_D( "SETUP DEFAULT DIRECTORIES" );
	createSessionDefaultDirectories();
	CI_LOG_D( "SETUP WORKING DIRECTORIES" );
	createSessionWorkingDirectories();

	CI_LOG_D( "SETUP OUTPUT WINDOW" );
	setupOutputWindow();
	CI_LOG_D( "SETUP OUTPUT FBO" );
	setupOutputFbo();

	CI_LOG_D( "SETUP PALETTES" );
	setupPalettes();

	CI_LOG_D( "SETUP CAMERA" );
	mCameraRef = EasyCamera::create( mOutputWindowRef );
	mCameraRef->enable();

	CI_LOG_D( "SETUP SAVERS" );
	setupImageSaver();
	setupSequenceSaver();

	CI_LOG_D( "SETUP DEBUG" );
	setupBoxBatch();
	setupGrid();

	CI_LOG_D( "SETUP PARTICLES" );
	setupParticles();
	CI_LOG_D( "SETUP PLEXUS" );
	setupPlexus();
	CI_LOG_D( "SETUP TRAIL" );
	setupTrail();

	CI_LOG_D( "SETUP POINTS" );
	setupPoints();
	CI_LOG_D( "SETUP SPRITES" );
	setupSprites();
	CI_LOG_D( "SETUP LINES" );
	setupLines();
	CI_LOG_D( "SETUP TRAIL POINTS" );
	setupTrailPoint();
	CI_LOG_D( "SETUP RIBBONS" );
	setupRibbon();
	CI_LOG_D( "SETUP PRIMITIVE" );
	setupPrimitive();
	CI_LOG_D( "SETUP POST" );
	setupPost();
	CI_LOG_D( "SETUP BG" );
	...
	CI_LOG_D( "SETUP AUDIO" );
	setupAudio();

	CI_LOG_D( "SETUP UI" );
	setupUIs();

	CI_LOG_D( "LOAD SETTINGS" );
	loadSettings( getAppSupportWorkingSessionPath() );
	CI_LOG_D( "DONE LOADING" );
}
...
void Transform::openEditor()
{
	...
	string cmd = "cd " + shaderPath.string();
#if defined( CINDER_MSW_DESKTOP )
	cmd += " && subl.exe .";
#else
	cmd += " && open -a /Applications/Sublime\\ Text.app .";
#endif
	system( cmd.c_str() );
}
